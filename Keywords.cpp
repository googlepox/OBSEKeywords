...
bool Cmd_HasAllKeywords_Execute(COMMAND_ARGS)
{
    *result = 0;
    ...
    // If we got here, all provided keywords were found
    *result = 1;
    return true;
}
...
// ===== ParamInfo Definitions =====

// Single form parameter (accepts any form type: NPCs, items, spells, etc.)
static ParamInfo kParams_OneForm[] = {
    { "form", kParamType_TESObject, 0 },
};

// Form + one string (keyword) parameter
static ParamInfo kParams_OneForm_OneString[] = {
    { "form",    kParamType_TESObject, 0 },
    { "keyword", kParamType_String,  0 },
};

// Form + index (for GetNthKeyword)
static ParamInfo kParams_GetNthKeyword[] = {
    { "form",  kParamType_TESObject, 0 },
    { "index", kParamType_Integer, 0 },
};

// Form + up to 4 keywords (for HasAnyKeyword / HasAllKeywords)
// keyword2-4 are optional (last param = 1 means optional)
